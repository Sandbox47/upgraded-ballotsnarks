...
- S. 18 (unten): Ich würde den Choice Space nicht als Teil der Instance sehen. Für mich wäre das eher ein Teil der Definition der Relation die der entsprechende ZKP beweist - so liest es sich auch in deiner Definition 2.5.4.
    ...
    Wie würde ich das dann visualisieren? (Was muss ich an Figure 2.6 ändern, damit nach wie vor klar ist, dass der Choice Space in der Definition des Proving Algorithmus verwendet wird. (Soll ich dann eventuell $Prove(C)$, $Prove_C$) machen? -> Ja und die Anmerkung "choice space is part of the instance" weglassen)
...
- S.19: Definition 2.5.3 ist sehr informal. Das ist an der Stelle erstmal nicht schlimm - ich wundere mich nur, wo das in dieser Form in [Gro16] steht, das du ja hierfür zitierst.
    ...
    - Add something for the zero-knowledge property, security properties such as computational (knowledge-)soundness, perfect completeness (state that these properties need to be fulfilled and what these properties mean intuitively)
    - Use introduction of zero-knowledge proof systems from moonmath manual as basis. (Start of chapter 8)
    Or take a look at the slides from Security and Privacy (see Mail)
...
- S.28:In Fußnote 2: Ich denke du meinst etwas anderes als du schreibst. Der Choicespace dürfte keine Teilmenge von G sein, oder?
    ...
    - The final encrypted tally is not necessarily compatible with the choice space but has entries that are still relatively small (add this)  (usually the set of allowed values is not more than $n$ times larger than the entires in the choice space if we have $n$ voters)
...
- Kapitel 4: Ich habe im Prinzip nichts dagegen, dass du nicht näher auf Weierstrass-Kurven eingehst. Denk aber dran, dass nur die ElGamal-Kurve (Curve25519 bzw. je nach Zeit noch soetwas wie Bandersnatch) eine Montgomery-Kurve sein wird, die SNARK-Kurve (BN254 oder BLS-312-81) aber keine Montgomerykurve ist. Je nachdem, wie viel du zu dem Thema später erzählst, kann es sein, dass du dir das Leben etwas schwer machst, wenn du Kapitel 4 auf Montgomery-Kurven einschränkst.
    ...
    Ich werde das dann entsprechend high-levelig behandeln bei Groth16. (Just say that I need a certain group that has a pairing map and scalar field -> example: BN254)
    - Solange ich kein konkretes Beispiel durchrechnen möchte, ist es doch eigentlich egal, was für eine Elliptische Kurve die Basis ist, oder? (Das wäre doch nur relevant, wenn ich tatsächlich das group law berechnen müsste, oder?) -> Ja
...
- Kapitel 4: Sind 4.4.2, 4.4.3, 4.4.4 und 4.4.6 nicht eher Sätze als Definitionen? Ich denke man müsste beweisen, dass xDbl(x(P))=x(P²) gilt (und analog für xAdd, die MontgomeryLadder und die y-recovery), oder zumindest den entsprechenden Beweis zitieren. 
    ...
    - Ich habe mal in dem Paper guguckt, an dem ich mich füür die Sektion orientiert habe. Da haben die Autoren auch nur auf Montgoemrey selbst verwiesen für die Pseudooperations. (Soll ich das auch machen?) -> Make proof or cite something
    - Für den Korrektkheitsbeweis der Montgomery Ladder habe ich jetzt das Übersichtspaper zitiert, an dem ich mich orientiert habe. Da ist einer gegeben. (-> Change this definition to theorem)
