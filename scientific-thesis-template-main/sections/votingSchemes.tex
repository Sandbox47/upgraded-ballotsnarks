...
\begin{definition}[Voter]
  Let $C$ be a non-empty choice space. Let $\mathcal{S}=(\text{Gen}(1^\eta), enc, dec)$ be a public-key (asymmetric) encryption scheme with:
  ...
  Let $zkp=(pro,ver)$ be the Zero-Knowledge protocol allowing a voter to proof ballot validity.
  Let $PBB=(pub, obt)$ be a public bulleting board.
  Then, a voter $V$ is defined by algorithm \ref{alg:voter}.
  \begin{Algorithmus} %Use the environment only if you want to place the algorithm similar to graphics from TeX
    \caption{Voter}
    \label{alg:voter}
    \begin{algorithmic}
    \Procedure{$V$}{$C$,$\mathcal{S}$, $pk$, $zkp$, $PBB$}
      \State $b_V \gets C$
