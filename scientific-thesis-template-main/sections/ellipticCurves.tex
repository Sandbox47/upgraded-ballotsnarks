...
\end{align*}
However, computing $P^n=[X_n:Y_n:Z_n]$ by iteratively using the group law $\odot_{M}^{Pro}$ $n-1$ times is computationally expensive as discussed in \cref{section:elGamalEncryption:precomputedPowers}.
...
This algorithm requires $\mathcal{O}(\log_2(n))$ many iterations to compute the coordinates $X_n$ and $Z_n$ of $P^n$ from the coordinates $X$ and $Z$ of $P$.
...
\begin{Algorithmus}[h] %Use the environment only if you want to place the algorithm similar to graphics from TeX
    ...
    \label{alg:xAdd}
    \begin{algorithmic}
    \Procedure{$xAdd$}{$x(P), x(Q), x(P\odot_M^{Pro}Q^{-1})$}
...
Analogously to Square and Multiply, we can compute exponentiations $P^n$ for some $P\in M_{A,B}(\mathbb{F}P^2)$ and $n\in\mathbb{N}$.
...
A proof of correctness of the $MontgomeryLadder$ is given in section 4.1 of \cite{costello_montgomery_2018}.
...
The algorithm $OkSakYRecovery$ we use here is based on Theorem 2 in \cite{goos_efficient_2001}. 
...
\begin{Algorithmus}[h] %Use the environment only if you want to place the algorithm similar to graphics from TeX
    ...
    \Procedure{$LadderYRecovery$}{$P, x(P^n), x(P^{n+1})$}
        ...
        \State $P_{rec}^n\gets [0:1:0]$
        \If{$Z_{P^{n+1}}=0$} \Comment \parbox[t]{0.7\linewidth}{%
                $\rightsquigarrow P^{n+1}=[0:1:0]$ and therefore $P^n=P^{-1}$.
            }
            \State $P_{rec}^n\gets [X_P:-Y_P:Z_P]$
        \Else
            \State $P_{rec}^n\gets OkSakYRecovery(P, x(P^n), x(P^{n+1}))$
        \EndIf
        \State $P_{rec}^n\gets NormProjective(P_{rec}^n)$
        \State \Return $P_{rec}$
    \EndProcedure
    \end{algorithmic}
\end{Algorithmus}
...
\begin{definition}[Montgomery Exponentiation]
    ...
    Let $P\in M_{A,B}(\mathbb{F}P^2)$.
    Let $n\in\mathbb{N}$, where $n=\sum\limits_{i=0}^{l-1}n_i2^i$ is a bit string of fixed length $l$.
    Then, we can compute $P^n=M\_Exp(P, (n_0, \dots, n_{l-1}))$ as described in \cref{alg:montgomeryExponentiation}.
\end{definition}
...
\begin{Algorithmus}[h] %Use the environment only if you want to place the algorithm similar to graphics from TeX
    ...
    \begin{algorithmic}
    \Procedure{$M\_Exp$}{$P, (n_0,\dots,n_{l-1})$}
        \State $(x(P^n), x(P^{n+1}))\gets  MontgomeryLadder(x(P), (n_0,\dots,n_{l-1}))$
        \State $P^n\gets LadderYRecovery(P, x(P^n), x(P^{n+1}))$
        \State \Return $P^n$
    \EndProcedure
    \end{algorithmic}
\end{Algorithmus}
...
\todo{
    Introduce Twisted Edwards curves and efficient group exponentiation with precomputed powers for TE-curve.
}
...
\begin{definition}[Twisted Edwards Curve]
    ...
    \begin{itemize}
        ...
        \begin{align*}
            (x_1, y_1)\odot_{TE} (x_2, y_2)=\left(\frac{x_1y_2+x_2y_1}{1+dx_1y_2y_1y_2},\frac{y_1y_2-ax_1x_2}{1-dx_1x_2y_1y_2}\right)
        \end{align*}
    \end{itemize}
...
\begin{Algorithmus}[h] %Use the environment only if you want to place the algorithm similar to graphics from TeX
    ...
    Here, $k=\sum\limits_{i=0}^{l-1}k_i\cdot 2^i$ is a bitstring of length $l$ and $g^{2^i}\:\forall i\in\{0,\dots, l-1\}$ are precomputed powers of $g$.}
    \label{alg:twistedEdwardsExp}
    \begin{algorithmic}
    \Procedure{$TE\_Exp$}{$\left(P^{2^i}\right)_{i=0}^{l-1}, \left(k_i\right)_{i=0}^{l-1}$}
        \State $y\gets (0,1)$ \Comment Neutral element
        \For{$i=0$ \textbf{ to } $l-1$}
            \If{$k_i=1$}
                $y\gets P^{2^i}\odot_{TE} y$
            \EndIf
        \EndFor
        \State \Return $y$
    \EndProcedure
