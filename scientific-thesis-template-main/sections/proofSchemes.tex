...
\begin{definition}[\gls{r1cs} Satisfiability]
    ...
    Let $S=(S_1,\dots, S_n)$ be an assignment to $R$.
    Then, we say that $S$ satisfies $R$, if and only if all $k$ equations from \cref{def:r1cs} are fulfilled.    

    We call $R$ satisfiable, if there is an assignment $S$ that satisfies $R$.
\end{definition}
...
Therefore, the \gls{r1cs} $R_{r_{3\text{-}vot}}$ should also be satisfied by $S$ but not by $S'$.
We can check this by inserting the values for the variables $S_1,\dots, S_7$ into the equations of $R_{r_{3\text{-}vot}}$:
\begin{itemize}
...
\begin{definition}[\gls{qap} Satisfiability]
    ...
    Let $(\mathbb{F},+,\cdot )$ be a field, let $n,k\in\mathbb{N}$.
    Let $a_j^i,b_j^i,c_j^i\in\mathbb{F}$ be constants from $\mathbb{F}$ for every index $i\in\{0,\dots, n\}$ and $j\in\{0,\dots, k\}$.
    Let $R$ be the corresponding \gls{r1cs} as in \cref{def:r1cs} and 
    \begin{align*}
        QAP(R)=\left\{T\in\mathbb{F}[x],\left\{A_j, B_j, C_j\in \mathbb{F}[x]\right\}_{j=0}^{n}\right\}
    \end{align*}
    ...
    \end{align*}
    Then, we say that $S$ satisfies $QAP(R)$, if and only if $P_S\mod T = 0$.

    We call $QAP(R)$ satisfiable, if there is an assignment $S$ that satisfies $QAP(R)$.
\end{definition}

As we have shown before, the \gls{r1cs} $R_{r_{3\text{-}vot}}$ is satisfied by the assignment $S=(0,0,1,0,0,0,0)$ but not by the assignment $S'=(1,1,0,0,0,0,0)$.
Therefore, the \gls{qap} $QAP(R_{r_{3\text{-}vot}})$ should also be satisfied by $S$ but not by $S'$.
...
We can formalize this concept as an Instance-Witness Decomposition.

\begin{definition}[Instance-Witness Decomposition]
    Let $(\mathbb{F}, +, \cdot)$ be a finite field.
    Let $r:\mathbb{F}^m\to\mathbb{F}^n$ be an algebraic function.
    Then, $R_r$ is the corresponding \gls{r1cs} and $QAP(R_r)$ is the corresponding \gls{qap}.
    The set of all assignments to $r$, $R_r$ and $QAP(R_r)$ is $\mathcal{A}=\mathbb{F}^{m+n}$.
    In an Instance-Witness decomposition, we split this set into one set for instance and witness each: $\mathcal{I}=\mathbb{F}^u,\: \mathcal{W}=\mathbb{F}^v$ with $u+v=m+n$.
    ...
    The Instance-Witness decomposition $IWD(rep)$ where $rep\in\{r,R_r, QAP(R_r)\}$ is then defined as 
    \begin{align*}
        IWD(rep)=(\mathcal{I},\mathcal{W},p,rep)\text{.}
    \end{align*}
    We say that an assignment $(I,W)=((I_1,\dots, I_u), (W_1,\dots, W_v))\in\mathbb{F}^u\times\mathbb{F}^v$ satisfies $IWD(rep)$, if and onyl if $S=p((I,W))$ satisfies $rep$.
    $IWD(rep)$ is called satisfiable, if there is an assignment $(I,W)\in\mathbb{F}^u\times\mathbb{F}^v$ such that $(I,W)$ satisfies $IWD(rep)$.
    %two index sets $\{i_1,\dots, i_u\}$ and $\{w_1,\dots, w_v\}$ for some $u,v\in\mathbb{N}$ with $u+v=m+n$.
\end{definition}

We can also visualize an Instance-Witness decomposition as in \cref{fig:circuit:instanceWitnessDecomposition}.
...
\begin{figure}[h]
    ...
    \begin{subfigure}{.48\textwidth}
        ...
        %\caption{Exemplary Instance-Witness decomposition of $r$.}
    \end{subfigure}
    \caption{Circuit for an algebraic function $r:\mathbb{F}^m\to\mathbb{F}^n$ and exemplary Instance-Witness decomposition of $r$.}
    \label{fig:circuit:instanceWitnessDecomposition}
\end{figure}
...
To compute the corresponding Instance-Witness decomposition, we choose $\mathcal{I}=\mathbb{F}_{13}^{4}$ and $\mathcal{W}=\mathbb{F}_{13}^3$.
Additionally, we choose the mapping $p$ as follows:
\begin{align*}
    p:\mathbb{F}_{13}^4\times \mathbb{F}_{13}^3\to\mathbb{F}_{13}^7, p((I_1, I_2, I_3, I_4), (W_1,W_2,W_3)) = (W_1,W_2,W_3,I_1,I_2,I_3,I_4)
\end{align*}
Then, $IWD(r)=(\mathcal{I}, \mathcal{W}, p, r)$ is the corresponding Instance-Witness decomposition.
Visually, we can represent this as shown in \cref{fig:circuit:instanceWitnessDecompositionThreeVot}.

\begin{figure}[h]
    \centering
    \includegraphics[scale=.8]{graphics/circuits/ExampleVoting_instanceWitnessDecomposition.pdf}
    \caption{Instance-Witness decomposition of the circuit for $r_{3\text{-}vot}$ if we choose the inputs as witness and the outputs as instance.}
    \label{fig:circuit:instanceWitnessDecompositionThreeVot}
\end{figure}

Let $IWD(QAP(R_{r_{3\text{-}vot}}))=(\mathcal{I}, \mathcal{W}, p, QAP(R_{r_{3\text{-}vot}}))$ be defined analogously to $IWD(r)$.
To prove that a ballot $b=(b_0,b_1,b_2)$ is in the choice space $C$ without revealing it, a voter can create the assignment $S=p((0,0,0,0),(b_0,b_1,b_2))=(b_0,b_1,b_2,0,0,0,0)$ to $QAP(R_{r_{3\text{-}vot}})$ and compute the corresponding polynomial $P_S$.
If $P_S$ is divisible by the target polynomial $T$ of $QAP(R_{r_{3\text{-}vot}})$, we know that the assignment $S$ satisfies $QAP(R_{r_{3\text{-}vot}})$ and therefore, that the ballot $b$ is valid.
