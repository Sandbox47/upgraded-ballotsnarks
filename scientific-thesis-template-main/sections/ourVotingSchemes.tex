...
This leads to the two different choices $\mathcal{S}=EEG(M_{126932,1}(\mathbb{F}_qP^2))$ or $\mathcal{S}:=PPEEG(TE_{126934, 126930}(\mathbb{F}_qP^2))$. \ref{footnote:ellipticCurveSubgroup}
The corresponding instantiations of the circuit $\mathfrak{C}_C^{Enc}$ are illustrated in \cref{fig:circuit:encMEEG} and \cref{fig:circuit:encTEPPEEG}.\footnote{
    ...
    Therefore, we need $i$ instantiations of $\mathfrak{C}_C^{Enc}$ respectively to assert correct encryption of all entries.
...
\centering
    ...
    The circuit utilizes $\mathfrak{C}_{EEG(M_{126932, 1}(\mathbb{F}_qP^2)).Enc}$ (\ref{fig:circuit:eegEnc}) as a building block.
    }
...
\begin{figure}[h]
    ...
    Here, we use a choice space $C\subseteq\mathbb{F}$.\cref{footnote:encryptionComponentwiseCircuit}
    The circuit utilizes $\mathfrak{C}_{EEG(TE_{126934, 126930}(\mathbb{F}_q)).Enc}$ (\ref{fig:circuit:ppeegEnc}) as a building block.
    }
...
The choice space $C$ in a voting system $VS_C$ only influences the relation $R_C$ and by extension the \gls{zps} instantiation but not any other part of the voting scheme.
...
\begin{definition}[Single-Vote System]
    ...
    \end{align*}
    The relation $r_{C_{\text{Single}, m}}^{Vot}:\mathbb{F}_q^m\to\emptyset$ asserts that a given ballot $b\in\mathbb{F}_q^{m}$ is in the choice space.
    To do so, the function asserts that all individual entries of $b$ are a bit and that the sum of all entries is a bit.
    Formally, let $b=(b_0,\dots, b_{m-1})\in\mathbb{F}_q^m$. Then, $r_{C_{\text{Single}}, m}^{Vot}$ operates as shown in \cref{alg:singleVoteVoting}.

    Here, the corresponding circuit $\mathfrak{C}_{C_{\text{Single}, m}}^{Vot}$ is satisfied by a ballot $b=(b_0,\dots, b_{m-1})\in\mathbb{F}_q^m$ if and only if $b\in C_{\text{Single}, m}$.
    We can represent this circuit graphically as in \cref{fig:circuit:singleVoteVoting}.

    The voting system based on the choice space $C_{\text{Singe}, m}$ is $VS_{C_{\text{Single}, m}}$.
\end{definition}
